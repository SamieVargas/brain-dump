...
| The reply is cut off before the JSON closes | 0 of 20 at 4,096; 483 of 500 native and 477 of 500 prompt at 1,024, and 6 of 20 at 2,048, 2026-09-22 | the parser reports `failed` with "cut off"; the Worker answers 502 with the parse path; the evals count `stop_reason: max_tokens` as a hard fail; on a streamed sort the page reads the stop reason and says "the sorter ran out of room before it finished", where it used to show the parser's "Unexpected end of JSON input" | the whole call's tokens for no plan |
