...
**Redesign, 2026-09-24.** The page and the sorter both speak the three-level, three-bucket contract (prompt `sort@v4` at effort `medium`, tuned from the first `sort@v3` run and measured against `low`, below), and the page sorts live against the Worker (`SORT_SOURCE = "live"` in `index.html`; `"sample"` swaps in a hand-sorted plan from a real 23 September dump, for a demo). The first `sort@v3` run is summarized under Evals; the older results there are from the five-state prompt (`sort@v2`) and stay as the baseline.
...
flowchart TD
  ...
  M["[model] Sort and re-plan call: claude-sonnet-5, effort medium, max_tokens 16,000,
...
**Re-planning.** Under the plan there are three pills, "I have 20 minutes", "move this to tomorrow" and "done with the top two", and they work on the page without another call. The Worker still takes a follow-up for the evals: it sends the conversation so far, the original dump as the first user turn, each plan as the assistant's own JSON, each follow-up as a user turn, capped to the last six turns with the current plan always kept. The Worker adds the one follow-up rule. The level can change mid-conversation and the caps and tone follow it. The plan lives on the page and in `sessionStorage`, so coming back to the tab shows it, and "save as PDF" under the plan opens a print-ready copy for the browser's print dialog to save: a white letter page with no filled areas, the three buckets as thin teal, amber and rose outlines with tick boxes to check off on paper, in portrait (stacked, later in two columns) or landscape (three columns, later capped at seven and "+ N more"), picked by the "vertical · horizontal" choice beside the button and remembered in `localStorage`.

**Speed.** A sort takes as long as the model takes to write it: across the 120 plans of the 2026-09-24 run, latency was about 1.5 s plus about 11 ms per output token (median 1,169 tokens and 14 s, the longest 3,803 tokens and 42.5 s), and output tokens include the model's thinking, which Sonnet 5 does by default at effort `high`. A real dump runs much longer than the eval dumps (the 23 September one is about 3,700 characters against a median of 181), which is where a 30-second sort comes from. So the Worker now sends `output_config.effort`, set by its `EFFORT` var (`medium` by default, `low` / `medium` / `high` allowed, reported by `GET /health`), and `node evals/run.js --effort=low` measures a level before it goes live. The page covers the wait with a sorting screen driven by the stream the Worker already forwards: up to five short lines from the person's own dump appear as chips ("reading your dump"), one chip at a time lights up on the model's thinking events ("thinking it through", the thinking itself is never shown), and once text arrives the chips dim and now, later and let go fill in as each item closes in the stream, the first now label typing in as it arrives, until "sorted." opens the plan. A small partial-JSON reader on the page closes whatever is still open after the last finished item, so each item lands whole. The header keeps the level and the elapsed seconds.
...
**Results, 2026-09-24, `claude-sonnet-5`, prompt `sort@v3`.** One pass of the new grid on the native contract: 20 dumps under all three levels with the anxious switch off and on (120 plans), plus the five follow-ups. File: `evals/results/2026-09-24-plenty+a_little+none-native-x1-sort-v3.md`, with its JSON beside it (restored from git after the same-day `sort@v4` run wrote over it; the runner now adds the prompt version to a stem instead).
...
Against `sort@v2` on the anxious state, "need to" went from 6 of 20 plans to 8 of 60 with anxious on, the mean cost per plan went from $0.0191 to $0.0134, and nothing was cut off. The routing rule for mental load is the one the page cannot fix on its own, so it was the first thing tuned: `sort@v4` asks the model to decide the shape before filling the buckets (a dump that is mostly weight gets exactly one gentle item at every level, the cap is a ceiling and not a target), to write actions and not the dump's "I need to", to reread every line for banned phrases before answering, and to never open a let_go statement with the name of a task it kept. The results are below.

**Results, 2026-09-24, `claude-sonnet-5`, prompt `sort@v4`, effort `medium` against `low`.** The same grid twice, one pass each on the native contract (120 plans and the five follow-ups per run). Files: `evals/results/2026-09-24-plenty+a_little+none-native-x1.md` (medium) and `…-x1-effort-low.md` (low), with their JSON beside them.

| Measure | `sort@v3`, no effort set (high) | `sort@v4`, medium | `sort@v4`, low |
| --- | --- | --- | --- |
| Latency per plan, median / p90 / longest | 14.0 / 23.8 / 42.5 s | 7.3 / 13.4 / 18.9 s | 4.6 / 6.6 / 10.3 s |
| Output tokens per plan, median | 1,169 | 569 | 312 |
| Cost per plan, mean | $0.0134 | $0.0071 | $0.0040 |
| Parse, cut off, cap | 120 of 120 native, 0 cut off, cap 100% | same | same |
| Banned phrasing (the page rewrites all of these) | 10 of 120 | 12 of 120 | 10 of 120 |
| Let go unique | 2 of 120 | 0 | 0 |
| Routing: more than one now item on a mental-load dump | 8 of 120 | 7 of 120 | 16 of 120 |
| Follow-ups that kept every item | 2 of 5 | 3 of 5 | 4 of 5 |

`sort@v4` at medium halves the wait and the cost of `sort@v3` and holds everything the page cannot fix. Low halves them again, but the model decides the shape less carefully: 16 mental-load dumps got two or three now items at plenty and a little (never at none, where the cap is one), against 7 at medium, and that is the one failure the page cannot correct. So the Worker stays on `EFFORT = "medium"`. The "need to" rewrite did not move (12 and 10 against 10), which is why the page's check stays on the request path; the duplicate rule did, from 2 to 0.
...
```
index.html                 the whole page: capture, levels, the sorting screen, the plan one task at a time, the timer, carry-over, the checks, save as PDF; one file, no build step
worker/
