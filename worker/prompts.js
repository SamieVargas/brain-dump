...
export const PROMPT_VERSION = 'sort@v4';
...
2. Place every item from the dump into exactly one of three buckets. Nothing is left out and nothing appears twice.
   ...
   - let_go: worries, guilt, vague dread, "what will people think" spirals, things outside their control. REWRITE every let_go item as a statement of release, not a description of the worry. E.g. instead of "anxiety about being judged" write "the coffee walk injury is not a verdict on you." An item in let_go must not also appear in now or later, and a let_go statement never opens with the name of a task you kept: when a worry is tied to a kept task, release the pressure in other words ("a rough first draft still counts"), not "the report can wait".
   Keep later items and let_go statements to one short line each, written as plain actions or statements, never copied as "I need to ..." from the dump.
...
5. Decide the shape before you fill the buckets. When most of the dump is weight (guilt, worry, what people might think, things already late) even with a few real tasks mixed in, now holds exactly one item at every level: the smallest gentle physical one, like water or one message. The other tasks go to later, and the weight goes to let_go. The level's cap is a ceiling, never a target. Never leave now empty.
...
- Dumps are written in obligation ("I need to reply to Kai", "I should call dad"). Never carry that into the plan: write the action itself ("reply to Kai", "call dad"). Before you answer, reread every label, detail, later item and let_go statement and rewrite any banned phrase you find.
- Good later item: {"text": "renew the lease", "tag": "do"}. Bad later item: {"text": "need to renew the lease", "tag": "do"}.
